...
def is_valid_api_key(api_key: Optional[str]) -> bool:
    """Checks if OPENAI_API_KEY is present and not a dummy placeholder."""
    if not api_key or not api_key.strip():
        return False
    key = api_key.strip().lower()
    if "your_openai_api_key" in key or key.startswith("sk-dummy") or key == "none":
        return False
    return True
...
def get_llm():
    """
    Factory function supplying LLM instance.
    Returns real ChatOpenAI if valid OPENAI_API_KEY is configured,
    otherwise explicitly logs and returns MockLLM for keyless local dev/testing.
    """
    settings = get_settings()
    
    if is_valid_api_key(settings.OPENAI_API_KEY):
        logger.info(f"[LLM_EXECUTION_MODE] Using REAL ChatOpenAI (model='{settings.OPENAI_MODEL}')")
        return ChatOpenAI(
        ...
        logger.info("[LLM_EXECUTION_MODE] OPENAI_API_KEY not configured or dummy key detected. Using MOCK Fallback LLM.")
        return MockLLM()
